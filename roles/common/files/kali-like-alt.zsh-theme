...
configure_prompt() {
    ...
    # DEFINA O PREFIXO E SUFIXO. ELES DEVEM SER TRANSPARENTES PARA A COR SER DEFINIDA PELO STATUS.
    ZSH_THEME_GIT_PROMPT_PREFIX="["
    # REMOVEMOS %{$RESET_COLOR%} DAQUI PARA GARANTIR QUE A COR SEJA RESETADA APENAS NO PROMPT
    ...
    # ESTADO SUJO (a branch e o símbolo ficam LARANJA/AMARELO)
    ZSH_THEME_GIT_PROMPT_DIRTY="%{$FG[214]%} *%{$LINEPROMPT%}"

    # ARQUIVOS MODIFICADOS
    ZSH_THEME_GIT_PROMPT_MODIFIED="%{$FG[196]%} !%{$LINEPROMPT%}"
    #  NÃO RASTREADOS
    ZSH_THEME_GIT_PROMPT_UNTRACKED="%{$FG[007]%} ✘%{$LINEPROMPT%}"

    # DEFINE CLOCK COLOR
    CLOCKPROMTP="$FG[214]"
    SUCCESSPROMPT="$FG[114]"
    ERRORPROMPT="$FG[196]"
    ...
    case "$PROMPT_ALTERNATIVE" in
        twoline)
            ...
            ;;
        oneline)
            PROMPT=$'$FGPROMPT%n@%m%b%F{reset}:$LINEPROMPT%~%b$(git_prompt_info)%F{reset}%(#.#.$) '
            RPROMPT=$'%{$LINEPROMPT%}(%(?.$SUCCESSPROMPT%?$LINEPROMPT.$ERRORPROMPT%?$LINEPROMPT)$LINEPROMPT)'
            ;;
    esac
}
